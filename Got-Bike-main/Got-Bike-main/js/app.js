...
window.showReceipt = function (bookingId) {
  db.ref('maycar_bookings').child(bookingId).once('value').then((snapshot) => {
    ...
    document.getElementById('receipt-booking-id').innerText = b.id;
    document.getElementById('receipt-car-img').src = b.carImage;
    document.getElementById('receipt-car-name').innerText = b.carName;
    ...
    document.getElementById('receipt-customer-name').innerText = b.customerName || '-';
    document.getElementById('receipt-customer-email').innerText = b.customerEmail || '-';
    document.getElementById('receipt-customer-phone').innerText = b.customerPhone || '-';
    document.getElementById('receipt-pickup').innerText = formatDate(b.pickupDate);
    document.getElementById('receipt-return').innerText = formatDate(b.returnDate);
    document.getElementById('receipt-total').innerText = '฿' + parseInt(b.totalAmount).toLocaleString();
...
function loadChatMessages() {
  ...
  const chatKey = userEmail.replace(/[.#$[\]]/g, '_');
    ...
    if (messages.length === 0) {
      messagesContainer.innerHTML = `
        <div class="chat-welcome-msg">
          ...
          <p>สวัสดีครับ! 👋<br>มีอะไรให้ช่วยไหมครับ? พิมพ์ข้อความได้เลย</p>
        </div>
      `;
      return;
    }
...
(function initChatBadgeListener() {
  ...
  // Check on load and when auth changes
